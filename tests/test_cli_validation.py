...
class TestCLIValidation:
    ...
    def test_required_arguments_process(self):
        """Test that process command requires report path."""
        parser = create_parser()

        with pytest.raises(SystemExit):
            parser.parse_args(["process"])  # Missing report path
    ...
    def test_required_arguments_extract(self):
        """Test that extract command requires report path."""
        parser = create_parser()

        with pytest.raises(SystemExit):
            parser.parse_args(["extract"])  # Missing report path
    ...
    def test_valid_argument_combinations(self):
        ...
        args = parser.parse_args(
            ["extract", "/test.pdf", "--llm-extractor", "llama3", "--json-output"]
        )
