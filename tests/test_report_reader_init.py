...
def _build_report_reader_without_heavy_init(*args, **kwargs) -> ReportReader:
    ...
    with ExitStack() as stack:
        stack.enter_context(patch("lx_anonymizer.report_reader.Faker", return_value=MagicMock()))
        stack.enter_context(
            patch("lx_anonymizer.report_reader.gender_detector.Detector", return_value=MagicMock())
        )
        stack.enter_context(
            patch("lx_anonymizer.report_reader.PatientDataExtractor", return_value=MagicMock())
        )
        stack.enter_context(
            patch("lx_anonymizer.report_reader.ExaminerDataExtractor", return_value=MagicMock())
        )
        stack.enter_context(
            patch("lx_anonymizer.report_reader.EndoscopeDataExtractor", return_value=MagicMock())
        )
        stack.enter_context(
            patch("lx_anonymizer.report_reader.ExaminationDataExtractor", return_value=MagicMock())
        )
        stack.enter_context(
            patch("lx_anonymizer.report_reader.SensitiveRegionCropper", return_value=MagicMock())
        )
        stack.enter_context(patch("lx_anonymizer.report_reader.Anonymizer", return_value=MagicMock()))
        # Force graceful fallback path and avoid external process/model startup.
        stack.enter_context(
            patch("lx_anonymizer.report_reader.ensure_ollama", side_effect=RuntimeError("test"))
        )
        stack.enter_context(
            patch("lx_anonymizer.report_reader.OllamaOptimizedExtractor", return_value=MagicMock())
        )
...
def test_report_reader_init_uses_passed_flags() -> None:
    ...
    assert reader.flags["patient_info_line"] == DEFAULT_SETTINGS["flags"]["patient_info_line"]
