...
class TestMainOllamaIntegration(unittest.TestCase):
    ...
    def test_main_with_pdf_and_ollama(self, mock_exists, mock_read_pdf):
        """Test main function with report input and Ollama analysis"""
        mock_exists.return_value = True
        mock_read_pdf.return_value = ("Sample report text with patient names", True)

        # Create a temporary test report (simulate)
        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp_file:
            tmp_file.write(b"fake pdf content")
            test_pdf_path = tmp_file.name

        try:
            # Test main function call with report
            result = main.main(
            ...
            # Verify report reading was called
