...
from cli.report_reader import ReportReaderCLI, create_parser, main
...
class TestReportReaderCLI:
    ...
    def test_create_reader(self, mock_report_reader, cli_instance):
        ...
        reader = cli_instance.create_reader(
            locale="de_DE", first_names=["John", "Jane"], last_names=["Doe", "Smith"]
        )
    ...
    def test_process_single_pdf_success(
        ...
        """Test successful single report processing."""
        # Setup mocks
    ...
    def test_process_single_pdf_file_not_found(self, cli_instance):
        """Test processing non-existent report file."""
        result = cli_instance.process_single_pdf(
            pdf_path="/nonexistent/file.pdf", verbose=False
        )
    ...
    def test_batch_process_directory_not_found(self, cli_instance, temp_dir):
        ...
        with pytest.raises(FileNotFoundError):
            cli_instance.batch_process(
                input_dir="/nonexistent/directory", output_dir=temp_dir
            )

    @patch("cli.report_reader.ReportReader")
    def test_extract_metadata_only(
        self, mock_report_reader, cli_instance, sample_pdf_path, sample_metadata
    ):
        """Test metadata-only extraction."""
        # Setup mock
        mock_reader_instance = Mock()
        mock_report_reader.return_value = mock_reader_instance
        mock_reader_instance.read_pdf.return_value = "report text content"
        mock_reader_instance.extract_report_meta_deepseek.return_value = sample_metadata

        # Test extraction
        result = cli_instance.extract_metadata_only(
            pdf_path=sample_pdf_path, use_llm_extractor="deepseek", json_output=False
        )
        ...
        mock_reader_instance.extract_report_meta_deepseek.assert_called_once_with(
            "report text content", sample_pdf_path
        )

    @patch("cli.report_reader.ReportReader")
    def test_extract_metadata_different_extractors(
        self, mock_report_reader, cli_instance, sample_pdf_path, sample_metadata
    ):
        ...
        for extractor, expected_method in zip(extractors, expected_methods):
            result = cli_instance.extract_metadata_only(
                pdf_path=sample_pdf_path, use_llm_extractor=extractor, json_output=False
            )
...
class TestCLIArgumentParsing:
    ...
    def test_parser_invalid_llm_extractor(self):
        ...
        with pytest.raises(SystemExit):
            parser.parse_args(
                ["process", "/path/to/file.pdf", "--llm-extractor", "invalid_extractor"]
            )
...
class TestCLIIntegration:
    ...
    def test_main_keyboard_interrupt(self, mock_argv):
        ...
        with patch(
            "cli.report_reader.ReportReaderCLI.process_single_pdf"
        ) as mock_process:
...
class TestErrorHandling:
    ...
    @patch("cli.report_reader.ReportReader")
    def test_process_pdf_with_exception(
        self, mock_report_reader, cli_instance, temp_dir
    ):
        """Test handling of exceptions during report processing."""
        # Create a test report file
        pdf_path = Path(temp_dir) / "test.pdf"
        pdf_path.touch()
        ...
        result = cli_instance.process_single_pdf(pdf_path=str(pdf_path), verbose=False)
    ...
    @patch("cli.report_reader.ReportReader")
    def test_batch_process_continue_on_error(
        self, mock_report_reader, cli_instance, temp_dir
    ):
        ...
        # Test batch processing
        results = cli_instance.batch_process(
            input_dir=str(input_dir), output_dir=str(output_dir), continue_on_error=True
        )
