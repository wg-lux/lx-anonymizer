...
try:
    ...
    from lx_anonymizer.frame_ocr_tesserocr import TesseOCRFrameProcessor
...
def create_test_frame_with_text():
    ...
    cv2.putText(frame, "Patient: Max Mustermann", (20, 30), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
    cv2.putText(frame, "Geburtsdatum: 15.03.1975", (20, 60), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
    cv2.putText(frame, "Fallnr: 12345", (20, 90), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
    cv2.putText(frame, "Datum: 13.08.2025", (20, 120), 
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
...
def benchmark_ocr_performance():
    ...
    if tesserocr_time > 0:
        ...
        print("=" * 30)
        print(f"🐌 pytesseract: {pytesseract_time:.3f}s ({pytesseract_time/num_frames:.3f}s/frame)")
        print(f"🚀 TesseOCR:    {tesserocr_time:.3f}s ({tesserocr_time/num_frames:.3f}s/frame)")
        ...
        print(f"🐌 pytesseract would take: {1800 * (pytesseract_time/num_frames):.1f} seconds")
        print(f"🚀 TesseOCR would take:    {1800 * (tesserocr_time/num_frames):.1f} seconds")
        print(f"⏰ Time saved:            {1800 * (pytesseract_time/num_frames) - 1800 * (tesserocr_time/num_frames):.1f} seconds")
    ...
    print(f"✅ Direct TesseOCR time: {direct_time:.3f}s ({direct_time/num_frames:.3f}s/frame)")
