...
This module extends the existing FrameOCR to identify and fix the root cause
of gibberish text in representative_ocr_text.
"""
...
class DiagnosticFrameOCR:
    ...
    def __init__(self, enable_diagnostics: bool = False, diag_dir: Optional[str] = None):
        ...
        Args:
            enable_diagnostics: Enable diagnostic logging and debugging
            diag_dir: Directory for diagnostic output files
        """
        ...
        # Frame sample counter for diagnostics
        self.frame_count = 0
        self.diag_samples = []
    ...
    def _get_robust_ocr_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'psm': int(os.getenv('OCR_PSM', '6')),  # Uniform text block
            'oem': int(os.getenv('OCR_OEM', '3')),  # Default LSTM
    ...
    def _check_tesseract_setup(self):
        ...
        try:
            ...
            logger.info(f"Languages available: {len(langs)} (deu: {has_deu}, eng: {has_eng})")
            
            if not has_deu:
                logger.warning("⚠️ German language pack (deu) not available!")
            if not has_eng:
                logger.warning("⚠️ English language pack (eng) not available!")
    ...
    def _analyze_text_quality(self, text: str) -> Dict[str, Any]:
        ...
        # Calculate gibberish score (0.0 = clean text, 1.0 = complete gibberish)
        gibberish_score = 0.0
        if punct_ratio > 0.5:
            gibberish_score += 0.4
    ...
    def _preprocess_for_ocr(self, frame: np.ndarray, roi: Optional[Dict] = None) -> np.ndarray:
        ...
        Based on diagnostic findings, this uses gentler preprocessing
        to avoid destroying text information.
        """
        try:
            ...
            if os.getenv('OCR_FIX_V1', '0') == '1':
                ...
                # Original processing (potentially problematic)
                processed = gray
    ...
    def extract_text_from_frame(
        ...
        Args:
            ...
            frame_id: Frame identifier for diagnostics
            
        Returns:
            Tuple of (text, confidence, ocr_data)
        """
        try:
            # Preprocess frame
            ...
            for i, word in enumerate(ocr_data['text']):
                if word.strip():
                    ...
                    if conf > 1:  # Minimum confidence threshold
    ...
    def _try_alternative_ocr(self, image: np.ndarray) -> Tuple[str, float]:
        ...
        try:
            ...
            alt_config = f"--oem 3 --psm 7 --dpi 400"
    ...
    def _log_diagnostic_sample(self, frame_id: int, processed_frame: np.ndarray, 
        ...
        if len(self.diag_samples) >= 20:  # Limit samples
            return
