...
class ReportReaderCLI:
    ...
    def process_single_pdf(
        ...
        """
        Process a single report file and return results.

        Args:
            pdf_path: Path to the report file
    ...
    def batch_process(
        ...
        """
        Batch process multiple report files in a directory.
        ...
        logger.info(f"Found {len(pdf_files)} report files to process")
    ...
    def extract_metadata_only(
        ...
        """
        Extract only metadata from a report without anonymization.
    ...
    def print_processing_summary(self, results: Dict[str, Any]):
        ...
        print("=" * 60)
        print(f"report File: {results['pdf_path']}")
        print(
            f"Original Text Length: {results.get('original_text_length', 0)} characters"
        )
        print(
            f"Anonymized Text Length: {results.get('anonymized_text_length', 0)} characters"
        )
    ...
    def print_batch_summary(self, results: List[Dict[str, Any]]):
        ...
        if failed:
            ...
            for result in failed:
                print(
                    f"  {Path(result['pdf_path']).name}: {result.get('error', 'Unknown error')}"
                )
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--locale", default="de_DE", help="Locale for text processing (default: de_DE)"
    )
    ...
    process_parser = subparsers.add_parser(
        "process", help="Process a single report file"
    )
    process_parser.add_argument("pdf_path", help="Path to the report file")
    ...
    process_parser.add_argument(
        "--ensemble", action="store_true", help="Use ensemble OCR for better accuracy"
    )
    process_parser.add_argument(
        "--llm-extractor",
    ...
    batch_parser = subparsers.add_parser(
        "batch", help="Batch process multiple report files"
    )
    batch_parser.add_argument("input_dir", help="Directory containing report files")
    batch_parser.add_argument(
        "--output-dir", "-o", required=True, help="Directory to save output files"
    )
    batch_parser.add_argument(
    ...
    extract_parser = subparsers.add_parser(
        "extract", help="Extract metadata only (no anonymization)"
    )
    extract_parser.add_argument("pdf_path", help="Path to the report file")
