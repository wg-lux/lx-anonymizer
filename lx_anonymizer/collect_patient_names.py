            ...
            for line_num, line in enumerate(f):
                ...
                try:
                    ...
                    if file_id and report_id:
                        ...
                        logger.warning(
                            f"Skipping line {line_num + 1} in {output_file}: Missing 'file' or 'report_id'."
                        )
                except json.JSONDecodeError:
                    logger.warning(
                        f"Skipping invalid JSON on line {line_num + 1} in {output_file}: {line[:100]}..."
                    )
    except Exception as e:
        logger.error(f"Failed to read existing results from {output_file}: {e}")
        return set()
    logger.info(
        f"Loaded {len(existing_ids)} existing record identifiers from {output_file}"
    )
    return existing_ids
...
def calculate_pdf_hash(report_file_path: Path) -> Optional[str]:
    ...
    # Construct the path to the corresponding report file
    pdf_file_path = report_file_path.with_suffix(".pdf")
    try:
        if not pdf_file_path.is_file():
            logger.error(
                f"Corresponding report file not found for {report_file_path.name}: {pdf_file_path}"
            )
            return None
        ...
        logger.error(
            f"Could not calculate report hash for {report_file_path.name}: [Errno 2] No such file or directory: '{pdf_file_path}'"
        )
        return None
    except Exception as e:
...
def process_report_file(
    report_file_path: Path, reader: ReportReader
) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Calculate report hash using the full path
        pdf_hash = calculate_pdf_hash(report_file_path)

        if isinstance(data, list):  # Handle cases where the JSON root is a list
            reports = data
        elif (
            isinstance(data, dict) and "reports" in data
        ):  # Handle cases where reports are under a "reports" key
            reports = data["reports"]
        else:
            logger.warning(
                f"Unexpected JSON structure in {report_file_path.name}. Expected list or dict with 'reports' key."
            )
            return []
        ...
        for report_data in reports:
            ...
            if header_line:
                ...
                logger.debug(
                    f"No header line found in report {report_id} of file {report_file_path.name}"
                )
...
def _process_json_file(
    ...
    if not isinstance(data, dict):
        logger.warning(
            "Skipping %s: Expected top-level JSON object to be a dictionary, found %s",
            fp,
            type(data).__name__,
        )
        return 0
    ...
    for report_id, obj in data.items():
        ...
        if llm_extractor:
            logger.info(
                f"Using LLM extractor '{llm_extractor}' for JSON report {report_id}"
            )
            ...
            if llm_extractor == "deepseek":
                ...
                logger.warning(
                    f"Unknown LLM extractor '{llm_extractor}' specified for JSON. Falling back."
                )
                extracted_meta = rr.extract_report_meta(raw_text, fp)

            if not extracted_meta:
                logger.warning(
                    f"LLM extractor '{llm_extractor}' failed for JSON report {report_id}. Falling back to SpaCy/Regex."
                )
                extracted_meta = rr.extract_report_meta(raw_text, fp)
        else:
            logger.info(
                f"Using default SpaCy/Regex extractor for JSON report {report_id}"
            )
            extracted_meta = rr.extract_report_meta(raw_text, fp)
        ...
        result_obj = {
            ...
            "dob": str(extracted_meta.get("patient_dob"))
            if extracted_meta.get("patient_dob")
            else None,
...
def _process_pdf_file(
    ...
    """
    Process a report if not in existing_ids, append result to output_file.
    ...
    original_text, anonymized_text, extracted_meta = rr.process_report(
        pdf_path=fp, use_llm_extractor=llm_extractor
    )

    if not extracted_meta:
        logger.warning(
            f"Metadata extraction failed for report {filename}. Skipping append."
        )
        return 0
...
def collect_names(folder: Path, output_file: Path, llm_extractor: str | None) -> None:
    ...
    for fp in tqdm(files_to_process, desc="Processing files", unit="file"):
        ...
        try:
            if fp.suffix.lower() == ".json":
                newly_added_count = _process_json_file(
                    rr, fp, llm_extractor, existing_ids, output_file
                )
            elif fp.suffix.lower() == ".pdf":
                newly_added_count = _process_pdf_file(
                    rr, fp, llm_extractor, existing_ids, output_file
                )
            else:
    ...
    logger.info(
        f"Finished processing folder. Added {total_new_results} new records in total to {output_file}"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Collect patient names from JSON/report reports using different extraction methods. Writes incrementally to JSON Lines (.jsonl)."
    )
    parser.add_argument("folder", type=Path, help="Folder with JSON/report reports")
    parser.add_argument(
        "--llm-extractor",
        type=str,
