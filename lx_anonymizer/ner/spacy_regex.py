    ...
    def extract_patient_info(self, text):
        ...
        if best_match:
            ...
            geb_indices = [
                ...
                if token.lower_ in ["geb", "geb.", "geboren"]
            ]
            fall_indices = [
                i
                for i, token in enumerate(span)
                if token.lower_ in ["fallnr", "fallnr.", "fallnummer"]
            ]
            date_indices = [
                i for i, token in enumerate(span) if DATE_RE.match(token.text)
            ]
            case_num_indices = [
                i
                for i, token in enumerate(span)
                if NUMBER_RE.match(token.text)
                and i > 0
                and span[i - 1].lower_ in [":", "fallnr", "fallnr.", "fallnummer"]
            ]
            ...
            if comma_indices:
                ...
                last_name_tokens = [
                    t.text for t in span[name_start_idx:comma_idx] if t.is_title
                ]
                if last_name_tokens:
                ...
                first_name_tokens = [
                    t.text for t in span[comma_idx + 1 : name_end_idx] if t.is_title
                ]
            ...
            if date_indices:
                ...
                if geb_indices:
                    ...
                    possible_dates = [
                        d_idx for d_idx in date_indices if d_idx > geb_idx
                    ]
                    if possible_dates:
                        date_idx = min(
                            possible_dates
                        )  # Take the first date after 'geb'
                ...
                if date_idx != -1:
                    ...
                    try:
                        ...
                        birthdate = (
                            None  # Mark as None instead of "INVALID DATE FORMAT"
                        )
            ...
            if case_num_indices:
                ...
                if fall_indices:
                    ...
                    possible_nums = [
                        c_idx for c_idx in case_num_indices if c_idx > fall_idx
                    ]
                    if possible_nums:
                        num_idx = min(
                            possible_nums
                        )  # Take the first number after 'fall'
    ...
    def examiner_data_extractor(self, text):
        ...
        if not self._examiner_ruler:
            patterns = [
                ...
                {
                    "label": "PER",
                    "pattern": r"Dr\.\s[A-Z][a-z]+\s[A-Z][a-z]+\s[A-Z][a-z]+\s[A-Z][a-z]+",
                },
                {
                    "label": "PER",
                    "pattern": r"Dr\.\s[A-Z][a-z]+\s[A-Z][a-z]+\s[A-Z][a-z]+\s[A-Z][a-z]+\s[A-Z][a-z]+",
                },
                {"label": "PER", "pattern": r"Dr\.med\.\s[A-Z][a-z]+"},
                {"label": "PER", "pattern": r"Dr\.med\.\s[A-Z][a-z]+\s[A-Z][a-z]+"},
                {
                    "label": "PER",
                    "pattern": r"Dr\.med\.\s[A-Z][a-z]+\s[A-Z][a-z]+\s[A-Z][a-z]+",
                },
                {
                    "label": "PER",
                    "pattern": r"Dr\.med\.\s[A-Z][a-z]+\s[A-Z][a-z]+\s[A-Z][a-z]+\s[A-Z][a-z]+",
                },
                {
                    "label": "PER",
