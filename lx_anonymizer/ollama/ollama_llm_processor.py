...
class OllamaLLMProcessor:
    ...
    def validate_and_classify_names(
        self, names: List[str], context: Optional[str] = None
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"JSON response not found {e}")
                pass
...
def analyze_full_image_with_ollama(
    image_path: Path, csv_path: Optional[Path] = None, csv_dir: Optional[Path] = None
    ...
    try:
        ...
        if csv_dir and name_entries:
            ...
            analysis_csv_path = (
                csv_dir
                / f"ollama_analysis_{image_path.stem}_{uuid.uuid4().hex[:8]}.csv"
            )
...
def replace_phi4_with_ollama(
    image_path: Path, existing_csv_path: Optional[Path] = None
) -> Optional[List[Dict]]:
    """
    ...
    try:
        ...
        name_entries, analysis_csv = analyze_full_image_with_ollama(
            image_path=image_path, csv_path=existing_csv_path, csv_dir=csv_dir
        )
        ...
        if name_entries:
            ...
            for entry in name_entries:
                logger.debug(
                    f"Found name: {entry.get('full_name', 'Unknown')} "
...
def initialize_ollama_processor(
    model_name: str = "llama3.2-vision:latest",
) -> Optional[OllamaLLMProcessor]:
    """
...
def analyze_text_with_ollama(
    text: str, csv_path: Optional[Path] = None, image_path: Optional[Path] = None
) -> List[str]:
    """
