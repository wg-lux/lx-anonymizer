...
def extract_frames(video_path: Path, output_dir: Path, max_frames: Optional[int] = None) -> List[Path]:
    """
...
def detect_sensitive_on_frame(frame_path: Path, report_reader) -> bool:
    """
...
def clean_video(
    ...
    try:
        ...
        frame_paths = extract_frames(video_path, frames_dir, max_frames=100)  # Limit for performance
        ...
        for i, frame_path in enumerate(frame_paths):
            if detect_sensitive_on_frame(frame_path, report_reader):
        ...
        success = remove_frames_from_video(
            video_path, 
            sensitive_frame_indices, 
            output_video,
