...
Uses specialized frame processing components separated from report logic.
"""
...
from lx_anonymizer.minicpm_ocr import (
    MiniCPMVisionOCR,
    _can_load_model,
    create_minicpm_ocr,
)
...
class FrameCleaner:
    ...
    def __init__(
        self, use_minicpm: bool = True, minicpm_config: Optional[Dict[str, Any]] = None
    ):
    ...
    def _stream_copy_with_pixel_conversion(
        ...
        try:
            ...
            cmd = [
                ...
                "-c:v",
                "libx264",  # Use efficient H.264 encoder
                "-preset",
                "ultrafast",  # Fastest encoding preset
                "-crf",
                "18",  # High quality constant rate factor
                "-c:a",
        ...
        try:
            ...
            if format_info["can_stream_copy"]:
                ...
                if "10le" in pixel_fmt or "422" in pixel_fmt:
                    ...
                    logger.info(
                        f"Fast re-encoding with stream copy audio: {input_video} -> {output_video}"
                    )
        ...
        try:
                ...
                if use_named_pipe and format_info["can_stream_copy"]:
                    ...
                    try:
                        ...
                        reader_cmd = [
                            ...
                            str(pipe_path),
                            "-c",
                            "copy",  # Pure stream copy from pipe
                            "-y",
                            str(output_video),
                        ]

                        logger.info(
                            f"Using named pipe for streaming mask: {crop_filter}"
                        )

                        # Start writer in background
                        writer_proc = subprocess.Popen(
                            writer_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
                        )
                        ...
                        try:
                            ...
                            try:
                                duration = (
                                    format_info.get("duration", 0) or 1
                                )  # seconds
                                multiplier = 10.0  # 10× realtime headroom (On server this might be unessecary)
                                reader_tmo = max(
                                    600, duration * multiplier
                                )  # at least 10 min
                                writer_tmo = (
                                    reader_tmo * 0.5
                                )  # writer should finish sooner
                            except KeyError:
                                # Fallback to default timeouts if duration not available
                                reader_tmo = 1000000
                                logger.warning(
                                    "FFProbe problem. Using default timeout for reader process"
                                )
                            ...
                            try:
                                writer_stdout, writer_stderr = writer_proc.communicate(
                                    timeout=writer_tmo
                                )
                                ...
                                if writer_returncode != 0:
                                    logger.error(
                                        f"Writer process failed with code {writer_returncode}: {writer_stderr.decode() if isinstance(writer_stderr, bytes) else writer_stderr}"
                                    )
                                    raise subprocess.CalledProcessError(
                                        writer_returncode, writer_cmd, writer_stderr
                                    )
                                ...
                                try:
                                    ...
                                    logger.error(
                                        "Writer process did not terminate gracefully, killing..."
                                    )
                                    writer_proc.kill()
                                    writer_proc.wait()
                                raise RuntimeError(
                                    "Named pipe writer process timed out"
                                )
                        ...
                        if writer_proc.poll() is None:  # Still running
                            logger.warning(
                                "Writer process still running during cleanup, terminating..."
                            )
                            writer_proc.terminate()
                            try:
                                writer_proc.wait(timeout=10)
                                ...
                                logger.error(
                                    "Writer process did not terminate, killing..."
                                )
                                writer_proc.kill()
                                writer_proc.wait()
                    ...
                    cmd = [
                        ...
                        crop_filter,
                        "-c:a",
                        "copy",  # Stream copy audio when possible
                        "-y",
                        str(output_video),
                    ]
            ...
            if output_video.exists() and output_video.stat().st_size > 0:
                ...
                if size_ratio < 0.1:  # Output suspiciously small
                    logger.warning(
                        f"Output video much smaller than input ({size_ratio:.1%})"
                    )

                logger.info(
                    f"Successfully created masked video: {output_video} (size ratio: {size_ratio:.1%})"
                )
                return True
            else:
    ...
    def clean_video(
        ...
        for idx, gray_frame, stride in self._iter_video(video_path, total_frames):
            ...
            if ocr_text:
                self.best_frame_text.push(ocr_text, ocr_conf, is_sensitive=is_sensitive)

            if is_sensitive:
                sensitive_idx.append(idx)

        # representative preview text
        accumulated["representative_ocr_text"] = self.best_frame_text.reduce().get(
            "best", ""
        )
    ...
    def detect_sensitive_on_frame_extended(
        ...
        try:
            ...
            if has_sensitive:
                logger.warning(
                    f"LLM detected sensitive data in frame {frame_path.name}: {meta}"
                )
                return True, meta
    ...
    def _mask_video(
        ...
        try:
            ...
            if endoscope_y == 0 and endoscope_h == mask_config.get(
                ...
                logger.info(
                    f"Using complex drawbox mask with {len(mask_filters)} regions"
                )
    ...
    def _create_mask_config_from_roi(
        ...
        logger.info(
            f"Created mask config from processor ROI: "
    ...
    def _update_video_sensitive_meta(self, video_file_obj, metadata: Dict[str, Any]):
        ...
        try:
            ...
            metadata = self.extract_metadata_deepseek(
                metadata.get("representative_ocr_text", "")
            )
            ...
            sensitive_meta, created = SensitiveMeta.objects.get_or_create(
                ...
                defaults={
                    ...
                    "representative_ocr_text": metadata.get(
            ...
            logger.info(
                f"{'Created' if created else 'Updated'} SensitiveMeta for video {video_file_obj.id}"
            )
    ...
    def analyze_video_sensitivity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            sensitive_frames = list(
                self._detect_sensitive_frames_streaming(total_frames)
            )
            logger.info(f"Detected {len(sensitive_frames)} sensitive frames")

            # Calculate sensitivity ratio
            sensitivity_ratio = (
                len(sensitive_frames) / total_frames if total_frames > 0 else 0.0
            )
            ...
            analysis_result = {
                ...
                "sensitive_frame_list": sensitive_frames[:100]
                if len(sensitive_frames) <= 100
                else sensitive_frames[:100] + ["...truncated"],
                "analysis_engine": "minicpm" if self.use_minicpm else "traditional",
            }

            logger.info(
                f"Analysis complete: {analysis_result['recommended_method']} recommended"
            )
