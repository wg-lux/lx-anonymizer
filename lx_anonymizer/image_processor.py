...
def process_image(
    ...
    """
    Process a single image or report page
    ...
    - text_extracted: Optional extracted text from report
    - skip_blur: Whether to skip blurring operations
    - skip_reassembly: Whether to skip report reassembly
    ...
    if text_extracted and not disable_llm:
        ...
        text_results, _ = analyze_text_with_phi4(
            text_extracted, csv_path=csv_path, image_path=img_path
        )
    ...
    if processed_image_path != img_path:
        ...
        try:
            if cv2.imread(str(processed_image_path)) is None:
                ...
                cv2.imwrite(str(result_path), cv2.imread(str(processed_image_path)))
