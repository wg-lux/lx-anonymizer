...
class TesseOCROptimized:
    ...
    def __init__(self, language: str = 'deu+eng'):
        ...
        import os
        tessdata_path = "/nix/store/3xz7i2zscqhfp70fylqs04cn8y02frfs-tesseract-5.5.1/share/tessdata"
    ...
    def process_image_boxes(
        ...
        with self._lock:  # Thread safety for the shared API
            for idx, box in enumerate(boxes):
                try:
                    ...
                    confidence_normalized = confidence_score if confidence_score > 0 else 0.0
                    
                    # Append results
                    extracted_text_with_boxes.append((ocr_result, box))
                    confidences.append(confidence_normalized)
                    
                    logger.debug(f"Processed box {idx + 1}/{len(boxes)}: "
        ...
        logger.info(f"TesseOCR processing complete: {len(boxes)} boxes in {processing_time:.3f}s "
    ...
    def get_performance_stats(self) -> dict:
        ...
        return {
            ...
            'boxes_per_second': self.processed_boxes / self.total_processing_time if self.total_processing_time > 0 else 0
        }
...
def compare_ocr_performance(
    ...
    # Test TesseOCR
    start_time = time.time()
    tesserocr_results, tesserocr_confidences = tesseract_on_boxes_fast(image_path, boxes, language)
    ...
    try:
        ...
        start_time = time.time()
        pytesseract_results, pytesseract_confidences = tesseract_on_boxes(image_path, boxes)
    ...
    comparison = {
        'tesserocr': {
            ...
            'avg_confidence': sum(tesserocr_confidences) / len(tesserocr_confidences) if tesserocr_confidences else 0,
        }
    }
    ...
    if pytesseract_time is not None:
        comparison['pytesseract'] = {
            ...
            'avg_confidence': sum(pytesseract_confidences) / len(pytesseract_confidences) if pytesseract_confidences else 0,
        }
        
        comparison['speedup'] = pytesseract_time / tesserocr_time if tesserocr_time > 0 else float('inf')
        
        logger.info(f"Performance comparison: TesseOCR is {comparison['speedup']:.1f}x faster than pytesseract")
