...
from lx_anonymizer.runtime_types import NamedOcrConfig as OCRConfig
...
class VideoCaptureLike(Protocol):
    ...
    def read(self) -> tuple[bool, ImageArray]: ...
...
class OCRDiagnostic:
    ...
    def check_tesseract_setup(self) -> dict[str, object]:
        ...
        try:
            ...
            logger.info(
                f"✅ Sprachen verfügbar: {len(langs)} (deu: {setup_info['has_deu']}, eng: {setup_info['has_eng']})"
            )
    ...
    def test_ocr_config(
        self, image: ImageArray, config: OCRConfig, frame_idx: int
    ) -> OCRResult:
    ...
    def preprocess_variations(self, frame: ImageArray) -> dict[str, ImageArray]:
        """Erstellt verschiedene Preprocessing-Varianten eines Frames."""
        variations: dict[str, ImageArray] = {}
        ...
        enhanced = cast(ImageArray, clahe.apply(gray))
    ...
    def diagnose_frame(
        self, frame: ImageArray, frame_idx: int, roi: OCRRoi | None = None
    ) -> list[OCRResult]:
        ...
        if roi:
            x, y, w, h = (
                roi.get("x", 0),
                roi.get("y", 0),
                roi.get("width", frame.shape[1]),
                roi.get("height", frame.shape[0]),
            )
    ...
    def generate_report(self, video_id: str, results: list[OCRResult]) -> str:
        ...
        with open(str(report_path), "w", encoding="utf-8") as f:
            ...
            f.write(
                f"- German language pack: {'✅' if setup.get('has_deu') else '❌'}\n"
            )
            f.write(
            ...
            f.write(
                "| Config | Preprocessing | Confidence | Readable Words | Punct Ratio | Text Sample |\n"
            )
            f.write(
                "|--------|---------------|------------|----------------|-------------|-------------|\n"
            )
            for _, row in best_configs.iterrows():
                f.write(
                    f"| {row['config_name']} | {row['preprocessing']} | {row['mean_conf']:.1f}% | {row['readable_words']} | {row['punct_ratio']:.3f} | {row['text_sample'][:30]}... |\n"
                )
            ...
            if df["mean_conf"].max() < 20:
                f.write(
                    "⚠️ **LOW CONFIDENCE ISSUE**: All OCR results have very low confidence (<20%)\n"
                )
            ...
            if df["punct_ratio"].mean() > 0.5:
                f.write(
                    "⚠️ **HIGH PUNCTUATION RATIO**: Text contains excessive punctuation/symbols\n"
                )
                f.write(
                    "- Likely indicating OCR misinterpretation of visual elements\n"
                )
                f.write("- Consider different PSM modes or preprocessing\n\n")

            if df["readable_words"].mean() < 2:
                f.write(
                    "⚠️ **LOW READABLE WORDS**: Very few meaningful words extracted\n"
                )
...
def main() -> None:
    ...
    parser.add_argument(
        "--num-frames", type=int, default=10, help="Number of frames to sample"
    )
    ...
    try:
        report_path = diagnostic.run_diagnosis(
            args.video_path, args.video_id, args.num_frames
        )
