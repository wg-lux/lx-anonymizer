...
def get_decoder_start_token_id(tokenizer):
    ...
    if hasattr(tokenizer, "get_decoder_start_token_id"):
        ...
        raise AttributeError(
            "Tokenizer does not have a method or attribute for decoder start token ID."
        )
...
def split_image_into_chunks(image, max_height=800, max_width=800, overlap=100):
    ...
    logger.info(
        f"Splitting image ({width}x{height}) into {num_chunks_h}x{num_chunks_w} chunks"
    )
    ...
    def find_better_split(start_pos, end_pos, is_vertical):
        ...
        search_start = max(
            start_pos + (max_height if is_vertical else max_width) // 2,
            end_pos - window,
        )
        ...
        if is_vertical:
            ...
            slice_values = [
                np.mean(img_array[y, :]) for y in range(search_start, search_end)
            ]
        else:
            # Looking for vertical lines (columns with high avg whitespace)
            slice_values = [
                np.mean(img_array[:, x]) for x in range(search_start, search_end)
            ]
        ...
        whitespace_positions = [
            i + search_start
            for i, v in enumerate(slice_values)
            if v > whitespace_threshold
        ]
        ...
        if large_image:
            ...
            for i, chunk in enumerate(chunks):
                ...
                try:
                    ...
                    logger.warning(
                        f"CUDA OOM on chunk {i + 1}, reducing chunk size and retrying"
                    )
                    try:
