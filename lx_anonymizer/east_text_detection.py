...
def east_text_detection(image_path, east_path=None, min_confidence=0.6, width=320, height=320):
    ...
    for i, (startX, startY, endX, endY) in enumerate(boxes):
        ...
        if min_width <= width <= max_width and min_height <= height <= max_height and width / height < 15:  # Aspect ratio check to avoid long text lines
            output_boxes.append((startX, startY, endX, endY))
            output_confidences.append({"startX": startX, "startY": startY, "endX": endX, "endY": endY, "confidence": float(confidences[i])})
