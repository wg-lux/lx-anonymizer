...
def load_field_usages(csv_path: Path) -> List[Dict[str, str]]:
    ...
    usages: List[dict[str | Any, str | Any]] = []
...
def find_closest_match(field: str, standard_fields: set[Any]) -> str:
...
def normalize_fields_in_files(usages: List[Dict[str, str]], root_dir: Path):
    ...
    for usage in usages:
        ...
        if normalized_field != keyword:
            print(
                f"Normalizing: {keyword} → {normalized_field} in {file_path}:{line_number}"
            )
            ...
            lines[line_number - 1] = add_missing_name_counterparts(
                lines[line_number - 1]
            )

            # Normalize the field and write back the updated line
            lines[line_number - 1] = normalize_code_line(
                lines[line_number - 1], keyword, normalized_field
            )
