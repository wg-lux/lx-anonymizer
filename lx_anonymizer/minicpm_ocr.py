...
def _can_load_model() -> bool:
    ...
    if storage_info['free_gb'] < min_storage_gb:
        logger.error(
            f"Insufficient storage: {storage_info['free_gb']:.1f}GB free, "
    ...
    if storage_info['hf_cache_gb'] > self.max_cache_size_gb:
        logger.warning(
            f"HuggingFace cache too large: {storage_info['hf_cache_gb']:.1f}GB, "
...
class MiniCPMVisionOCR:
    ...
    def _check_and_manage_storage(self):
        ...
        logger.info(
            f"Storage check: {storage_info['free_gb']:.1f}GB free, "
        ...
        if storage_info['free_gb'] < self.min_storage_gb:
            if self.auto_cleanup:
                logger.warning(
                    f"Low storage ({storage_info['free_gb']:.1f}GB < {self.min_storage_gb}GB), "
                    "attempting cleanup..."
                )
        ...
        if storage_info['hf_cache_gb'] > self.max_cache_size_gb:
            if self.auto_cleanup:
                logger.warning(
                    f"HF cache too large ({storage_info['hf_cache_gb']:.1f}GB > "
                    f"{self.max_cache_size_gb}GB), cleaning up..."
                )
                self._cleanup_hf_cache()
            ...
            if repos_to_delete:
                ...
                for repo in repos_to_delete:
                    try:
                        repo.delete()
                        logger.info(
                            f"Deleted cached model: {repo.repo_id} "
                            f"({repo.size_on_disk / 1024**3:.1f}GB)"
                        )
                    except Exception as e:
    ...
    def _manual_cache_cleanup(self) -> float:
        ...
        try:
            ...
            freed_gb = (size_before - size_after) / (1024**3)
            logger.info(
                f"Manual cache cleanup completed: {files_deleted} files deleted, "
    ...
    def _load_model(self):
        ...
        try:
            ...
            if storage_info['free_gb'] < estimated_model_size + 5:
                logger.warning(
                    f"Insufficient storage for model loading. "
                    f"Available: {storage_info['free_gb']:.1f}GB, "
            ...
            logger.info(
                f"MiniCPM-o 2.6 model loaded successfully. "
