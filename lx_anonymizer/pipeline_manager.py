...
def process_images_with_OCR_and_NER(
    ...
    - skip_reassembly: bool
        Skip reassembling the report after processing. Default is False.
    """
    ...
    try:
        ...
        if extracted_text and skip_blur:
            ...
            text_results, _ = analyze_text_with_phi4(
                extracted_text, text_csv_path, file_path
            )
        ...
        for img_path in image_paths:
            ...
            if skip_blur:
                ...
                image_results, _ = analyze_full_image_with_context(
                    img_path, llm_csv_path, csv_dir
                )
                logger.info(
                    f"LLM image analysis results: {len(image_results) if image_results else 0} entities"
                )
            else:
            ...
            all_text_detection_confidences = (
                east_confidences + tesseract_confidences + craft_confidences
            )
            all_ocr_results = filter_empty_boxes(all_ocr_results)

            # If you need it as JSON later:
            all_text_detection_confidences_json = json.dumps(
                all_text_detection_confidences
            )
        ...
        llm_results, llm_csv_path = analyze_full_image_with_context(
            file_path, csv_path, csv_dir
        )
        result["llm_results"] = llm_results
        result["llm_csv_path"] = llm_csv_path

        logger.info(
            f"Processing completed: {len(combined_results)} results, csv: {csv_path}, llm results: {llm_csv_path}"
        )
