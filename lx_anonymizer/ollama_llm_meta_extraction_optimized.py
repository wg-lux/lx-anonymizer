        ...
        try:
            ...
            try:
                ...
                if confidence >= confidence_threshold:
                    metadata = PatientMetadata(**metadata_dict)
                    logger.info(f"✅ Smart-Sampling erfolgreich (Konfidenz: {confidence:.2f}): {metadata.patient_name}, Alter: {metadata.patient_age}")
                    return metadata
                else:
...
class VideoMetadataEnricher:
    ...
    def enrich_from_multiple_sources(
        self, video_path: str, frame_samples: List[Any] = None, ocr_texts: List[str] = None, existing_metadata: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
