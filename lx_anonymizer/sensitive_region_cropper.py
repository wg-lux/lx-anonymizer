...
class SensitiveRegionCropper:
    ...
    def detect_sensitive_regions(self, 
                                image: Image.Image, 
                                word_boxes: List[Tuple[str, Tuple[int, int, int, int]]]) -> List[Tuple[int, int, int, int]]:
        """
        ...
        for data_type, pattern in self.sensitive_patterns.items():
            ...
            for match in matches:
                ...
                logger.debug(f"Gefunden {data_type}: '{matched_text}' an Position {start_pos}-{end_pos}")
    ...
    def _find_word_boxes_for_text_span(self, 
        ...
        for i, (word, bbox) in enumerate(word_boxes):
            if word.lower() in target_words:
                ...
                while (j < len(word_boxes) and 
                       word_idx < len(target_words) and
                       word_boxes[j][0].lower() == target_words[word_idx]):
                    matching_boxes.append(word_boxes[j][1])
                    j += 1
                    word_idx += 1
                
                if len(matching_boxes) >= len(target_words) / 2:  # Mindestens 50% der Words gefunden
                    break
        
        return matching_boxes

    def _find_patient_data_regions(self, 
                                  patient_info: Dict, 
                                  word_boxes: List[Tuple[str, Tuple[int, int, int, int]]],
                                  full_text: str) -> List[Tuple[int, int, int, int]]:
        """
    ...
    def _regions_should_merge(self, region1: Tuple[int, int, int, int], region2: Tuple[int, int, int, int]) -> bool:
        """
        ...
        return (horizontal_distance <= self.merge_distance or 
                vertical_distance <= self.merge_distance)

    def crop_sensitive_regions(self, 
                              pdf_path: str, 
                              output_dir: str,
                              page_numbers: Optional[List[int]] = None) -> Dict[str, List[str]]:
        """
    ...
    def visualize_sensitive_regions(self, 
                                   image: Image.Image, 
                                   word_boxes: List[Tuple[str, Tuple[int, int, int, int]]],
    ...
    def create_anonymized_pdf_with_crops(self, 
                                        pdf_path: str, 
                                        crop_output_dir: str,
                                        anonymized_pdf_path: str) -> None:
        """
...
def crop_sensitive_regions_from_pdf(pdf_path: str, 
                                   output_dir: str, 
                                   margin: int = 20,
                                   visualize: bool = False) -> Dict[str, List[str]]:
    """
