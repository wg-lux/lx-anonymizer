...
Separated from report processing to maintain clean architecture.
"""
...
class FrameOCR:
    ...
    def preprocess_frame_for_ocr(
        self, frame: np.ndarray, roi: Optional[Dict[str, Any]] = None
    ) -> Image.Image:
        """
        ...
        try:
            ...
            if len(frame.shape) == 3:
                return Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)).convert(
                    "L"
                )
            else:
    ...
    def extract_text_from_frame(
        ...
        try:
            ...
            logger.debug(
                f"Frame OCR extracted {len(words)} words with {avg_confidence:.2f} confidence"
            )
    ...
    def detect_sensitive_content(
        ...
        try:
            ...
            ocr_text, confidence, _ = self.extract_text_from_frame(
            ...
            has_sensitive = any(
                [patterns["patient_names"], patterns["dates"], patterns["case_numbers"]]
            )
    ...
    def _validate_roi(self, roi: Dict[str, Any]) -> bool:
        ...
        try:
            x, y, w, h = roi["x"], roi["y"], roi["width"], roi["height"]
            return (
                all(isinstance(v, (int, float)) and v >= 0 for v in [x, y, w, h])
                and w > 0
                and h > 0
            )
    ...
    def process_frame_batch(
        self, frames: list[np.ndarray], roi: Optional[Dict[str, Any]] = None
        ...
        for i, frame in enumerate(frames):
            try:
                ...
                _, confidence, _ = self.extract_text_from_frame(
                    frame, roi, high_quality=False
                )
