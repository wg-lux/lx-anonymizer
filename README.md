...
~/etc/lx-anonymizer/data # will hold csv results and final images generated
~/etc/lx-anonymizer/temp # can be cleaned regularly

*CHANGE THIS IN: lx_anonymizer/directory-setup.py*
...
Additionally, RegEx with spacy is use to extract  This tool is particularly useful in scenarios where sensitive information needs to be redacted from images or documents while retaining the overall context and visual structure.
...
git clone https://github.com/maxhild/lx-anonymizer.git
cd lx-anonymizer
direnv allow
...
The package will also available on pip through:
...
3. **CRAFT Text Detection** Uses CRAFT Text detection to detect text regions.

All methods can be used as part of an anonymization pipeline to detect text regions for further processing (e.g., blurring, pseudonymization).

The main function will in standard mode use all three methods to extract boxes.
...
## LLM (Phi-3.5) Full Text Analysis Module 

The LLM (Language Model) module adds an additional layer of name detection and validation using the Phi-4 model. This module is supposed to analyze both the OCR results and the broader context of the document to improve name detection accuracy.
Key Components

Text Analysis:

Processes full image OCR text from Tesseract
Incorporates box-specific OCR results and confidence scores
Uses contextual information from the generated CSV files


Structured Output:

Provides detailed name analysis including:

Detected full names (FirstName LastName format)
Box coordinates where names were found
Role classification (Patient/Staff/Unknown)
Confidence levels (High/Medium/Low)


Integration:

Works alongside traditional NER (SpaCy and Flair)
Outputs results to a separate CSV for validation
Can be used to verify or supplement other name detection methods


Performance Optimization:

Uses CUDA acceleration when available
Implements memory management for GPU resources
Maintains separate context for batch processing
